...
final class CommentaireController extends AbstractController
{
    ...
    public function new(Request $request, EntityManagerInterface $em, #[CurrentUser] Utilisateur $utilisateur): JsonResponse
    {
        ...
        if ($role === Role::ALTERNANT->name) {
            throw $this->createAccessDeniedException("Vous n'avez pas le droit de créer un commentaire.");
        }
        ...
        if (!$fiche) {
            throw new BadRequestHttpException("La fiche spécifiée n'existe pas");
        }

        if (!$this->isGranted('EDIT', $fiche)) {
            throw $this->createAccessDeniedException("Vous n'avez pas le droit de commenter cette fiche.");
        }

        // On crée un commentaire
        $commentaire = new Commentaire();
        $commentaire->setCommentaire($commentaire_data);
    ...
    public function edit(Request $request, ?Commentaire $commentaire, EntityManagerInterface $entityManager, #[CurrentUser] Utilisateur $utilisateur): JsonResponse
    {
        ...
        if (null === $commentaire) {
            throw $this->createNotFoundException("Le commentaire spécifié n'existe pas.");
        }
    ...
    public function delete(#[CurrentUser] Utilisateur $utilisateur, ?Commentaire $commentaire, EntityManagerInterface $entityManager): JsonResponse
    {
        // Seul l'auteur ou une personne ayant le rôle ADMIN peut modifier
        if ($commentaire?->getAuteur() !== $utilisateur && !$this->isGranted('ROLE_ADMIN')) {
